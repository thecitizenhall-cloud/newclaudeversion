      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"32px 24px" }}>
        ...
        <div style={{ display:"flex", gap:0, marginBottom:24, borderBottom:`1px solid ${T.border}` }}>
          {[
            ...
            { key:"reports",   label:"Reported posts",        count:reports.filter(r=>r.status==="pending").length,   color:T.redHi },
          ].map(t => (
            <div key={t.key} onClick={()=>setTab(t.key)} style={{
              ...
              {t.label}
              {t.count > 0 && <span style={{ background:T.surface, color:t.color, border:`1px solid ${t.color}44`, borderRadius:99, padding:"0 7px", fontSize:10 }}>{t.count}</span>}
            </div>
          ))}
        </div>
          ...
          <div key={app.id} style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:12, marginBottom:16, overflow:"hidden", animation:"fadeUp 0.3s ease both", animationDelay:`${i*0.05}s` }}>
            <div style={{ padding:"16px 20px", borderBottom:`1px solid ${T.border}` }}>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:6 }}>
                ...
                <StatusBadge status={app.status}/>
                {app.civic_verified && (
                  <span style={{ padding:"2px 9px", borderRadius:99, fontSize:10, fontWeight:500, background:T.tealLo, color:T.tealHi, border:`1px solid ${T.teal}44` }}>✓ Civic API match</span>
                )}
              </div>
          ...
          {reports.filter(r => filter === "all" || r.status === filter).map((r, i) => (
            <div key={r.id} style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:12, marginBottom:16, overflow:"hidden", animation:"fadeUp 0.3s ease both", animationDelay:`${i*0.05}s` }}>
              <div style={{ padding:"14px 20px", borderBottom:`1px solid ${T.border}` }}>
                ...
                <div style={{ fontSize:11, color:T.creamFaint }}>Reported by {r.reporter?.email || "unknown"}</div>
              </div>
              {r.status === "pending" && (
